...
int ZSudoku::OneLineModule(DIRECTION emDir, int nIndex)
{
	int nCount = 0;
	std::vector<int> vec_nReverseIndex;
	for (int i = 0; i < ZSUDOKU_SIZE; ++i)
	{
		switch (emDir)
		{
	...
	for (int nNum = 1; nNum <= 9; ++nNum)
	{
		...
		{
			std::vector<CPoint> vec_pt;
			for (const int& nReverseIndex : vec_nReverseIndex)
			{
				switch (emDir)
				{
				case DIR_ROW:
				{
					BLOCKINDEX emBlock = BLOCKINDEX(nIndex / 3 * 3 + nReverseIndex / 3);
					if (m_sz2_nData[nIndex][nReverseIndex] == 0 && GetNumCount(nNum, DIR_COLUMN, nReverseIndex) == 0 && GetNumCount(nNum, emBlock) == 0)
						vec_pt.push_back(CPoint(nIndex, nReverseIndex));
				}
				break;
				case DIR_COLUMN:
				{
					BLOCKINDEX emBlock = BLOCKINDEX(nReverseIndex / 3 * 3 + nIndex / 3);
					if (m_sz2_nData[nReverseIndex][nIndex] == 0 && GetNumCount(nNum, DIR_ROW, nReverseIndex) == 0 && GetNumCount(nNum, emBlock) == 0)
						vec_pt.push_back(CPoint(nReverseIndex, nIndex));
				}
				break;
				default:break;
				}
...
int ZSudoku::NineBlockModule(BLOCKINDEX emBlock)
{
	int nCount = 0;
	CPoint ptTest;
	std::vector<CPoint> vec_ptGroup;
	for (int i = 0; i < 3; ++i)
	{
		for (int j = 0; j < 3; ++j)
		{
			ptTest.x = (emBlock / 3) * 3 + i;
			ptTest.y = (emBlock % 3) * 3 + j;
			if (m_sz2_nData[ptTest.x][ptTest.y] == 0)
				vec_ptGroup.push_back(ptTest);
		}
	}
	for (int nNum = 1; nNum <= 9; ++nNum)
	{
		...
		{
			std::vector<CPoint> vec_pt;
			for (const CPoint& pt : vec_ptGroup)
			{
				if (m_sz2_nData[pt.x][pt.y] == 0 && GetNumCount(nNum, DIR_ROW, pt.x) == 0 && GetNumCount(nNum, DIR_COLUMN, pt.y) == 0)
					vec_pt.push_back(pt);
			}
			if (vec_pt.size() == 1)
			{
...
int ZSudoku::OneLineAlgorithm()
{
	int nCount = 0;
	for (int i = 0; i < ZSUDOKU_SIZE; ++i)
	{
		nCount += OneLineModule(DIR_ROW, i);
		nCount += OneLineModule(DIR_COLUMN, i);
	}
	return nCount;
}


int ZSudoku::NineBlockAlgorithm()
{
	int nCount = 0;
	for (int i = 0; i < ZSUDOKU_SIZE; ++i)
		nCount += NineBlockModule((BLOCKINDEX)i);
	return nCount;
}
...
{
	return (ThreeLinesAlgorithm() + OneLineAlgorithm() + NineBlockAlgorithm() + CandidateAlgorithm());
}
...
bool ZSudoku::CandidateSel(int nCandSize)
{
	for (int i = 0; i < ZSUDOKU_TOTAL_SIZE; ++i)
	{
		int nX = i / ZSUDOKU_SIZE;
		int nY = i % ZSUDOKU_SIZE;
		if (m_sz2_vec_nData[nX][nY].size() == nCandSize)
		{
			m_sz2_nData[nX][nY] = m_sz2_vec_nData[nX][nY][rand() % nCandSize];
			m_nTotal++;
			return true;
		}
	}
