...
private:
	...
	int OneLineModule(DIRECTION emDir, int nIndex);
	int NineBlockModule(BLOCKINDEX emBlock);
	int CandidateModule(int nX, int nY);
	int ThreeLinesAlgorithm();
	int OneLineAlgorithm();
	int NineBlockAlgorithm();
	...
	bool CandidateSel(int nCandSize);
	bool RandomCalc();
public:
	ZSudoku();
	virtual ~ZSudoku();
